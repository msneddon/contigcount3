...
def get_config():
    ...
    for nameval in config.items(get_service_name() or 'SneddonContigCounter'):
        retconfig[nameval[0]] = nameval[1]
    return retconfig

config = get_config()

from SneddonContigCounter.SneddonContigCounterImpl import SneddonContigCounter
impl_SneddonContigCounter = SneddonContigCounter(config)
...
async_run_methods = {}
async_check_methods = {}
async_run_methods['SneddonContigCounter.count_contigs_async'] = ['SneddonContigCounter', 'count_contigs']
async_check_methods['SneddonContigCounter.count_contigs_check'] = ['SneddonContigCounter', 'count_contigs']
sync_methods['SneddonContigCounter.count_contigs'] = True
...
class Application(object):
    ...
    def __init__(self):
        submod = get_service_name() or 'SneddonContigCounter'
        ...
        self.rpc_service.add(impl_SneddonContigCounter.count_contigs,
                             name='SneddonContigCounter.count_contigs',
                             types=[basestring, basestring])
        self.method_authentication['SneddonContigCounter.count_contigs'] = 'required'
                    ...
                    if auth_req != "none":
                        if token is None and auth_req == 'required':
                            ...
                            err.data = "Authentication required for " + \
                                "SneddonContigCounter but no authentication header was passed"
                            raise err
