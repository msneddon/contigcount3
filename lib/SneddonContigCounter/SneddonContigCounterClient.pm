package SneddonContigCounter::SneddonContigCounterClient;
...
SneddonContigCounter::SneddonContigCounterClient

=head1 DESCRIPTION


A KBase module: SneddonContigCounter
...
sub new
{
    ...
    my $self = {
	client => SneddonContigCounter::SneddonContigCounterClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$workspace_name is a SneddonContigCounter.workspace_name
$contigset_id is a SneddonContigCounter.contigset_id
$return is a SneddonContigCounter.CountContigsResults
...
# Authentication: required
	...
	method => "SneddonContigCounter.count_contigs",
	params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub version {
    my ($self) = @_;
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "SneddonContigCounter.version",
        params => [],
    });
    if ($result) {
        if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for SneddonContigCounter::SneddonContigCounterClient\n";
    }
    if ($sMajor == 0) {
        warn "SneddonContigCounter::SneddonContigCounterClient version is $svr_version. API subject to change.\n";
    }
}
...
package SneddonContigCounter::SneddonContigCounterClient::RpcClient;
